...
For example, 153 is a 3-narcissistic number (153 = 1^3^ + 5^3^ + 3^3^). 
...
To calculate the[ golden ratio $\phi$](https://en.wikipedia.org/wiki/Golden_ratio) , we can first observe the [Fibonacci sequence](https://en.wikipedia.org/wiki/Fibonacci_number): 1 , 1 , 2 , 3 , 5 , 8 , 13 , 21 , ．．． .  In this sequence, the upcoming number is found by adding up the two numbers before itself, that is, _F~n~_ = _F~n-1~_ + _F~n-2~_ . While _n_ becomes larger, the ratio $\frac{F_n}{F_n-_1}$ converges to golden ratio as _n_ goes to infinity.
...
**Note:** 
1. You don't need to consider the overflow problem, please use ```ulong``` instead of ```int``` to declare _F~n~_ , _F~n-1~_ and _F~n-2~_ .
